...
from mast.core.hosts import HostEntry, load_hosts, save_hosts
from mast.qt6.hosts.dialogs import HostsEditDialog
...
class HostsWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["", _("IP Address"), _("Hostname"), _("Comment")]
        )
        ...
        self.table.setColumnWidth(
            2, self.table.fontMetrics().horizontalAdvance("M") * 32
        )  # ~32 chars
        self.table.horizontalHeader().setSectionResizeMode(
    ...
    def load_hosts(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                _("Error"),
                _("Cannot read {0}. Root permission required.").format(HOSTS_FILE),
            )
            return
        except FileNotFoundError:
            QMessageBox.warning(
                self, _("Error"), _("{0} not found.").format(HOSTS_FILE)
            )
            return
        ...
        for row, entry in enumerate(self.hosts_entries):
            ...
            if entry.editable:
                ...
                checkbox.stateChanged.connect(
                    lambda s, r=row: self.toggle_enabled(r, s)
                )
    ...
    def add_host(self) -> None:
        ...
        if dialog.exec():
            ...
            if ip and hostnames:
                ...
                QMessageBox.warning(
                    self, _("Error"), _("IP address and hostname are required.")
                )
    ...
    def edit_host(self) -> None:
        ...
        dialog = HostsEditDialog(
            self, entry.ip, " ".join(entry.hostnames), entry.comment
        )
        if dialog.exec():
            ...
            if new_ip and new_hostnames:
                ...
                new_comment_sep = (
                    entry.comment_sep
                    if entry.comment_sep
                    else ("\t# " if new_comment else "")
                )
    ...
    def save_hosts(self) -> None:
        ...
        if result == "ok":
            QMessageBox.information(
                self, _("Success"), _("Hosts file saved successfully.")
            )
            ...
            QMessageBox.critical(
                self,
                _("Error"),
                _("Cannot write to {0}. Root permission required.").format(HOSTS_FILE),
            )
